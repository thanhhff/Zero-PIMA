...
class RoIHeads(nn.Module):
    ...
    ######## THANHNT_UPDATE: START 
    def select_target_samplse(self, proposals, targets, labels):
    ...
    def select_target_samplse_random(self, proposals, targets, labels):
        device = proposals[0].device
        new_proposals = list(proposals)
        new_labels = list(labels)

        num_images = len(new_proposals)
        for img_id in range(num_images):
            # Select all positive samples
            pos_inds = torch.nonzero(new_labels[img_id] > 0).squeeze(1)
            neg_inds = torch.nonzero(new_labels[img_id] == 0).squeeze(1)
            # get random neg_inds with length = len(pos_inds)
            neg_inds = torch.randperm(len(neg_inds))[:len(pos_inds)].to(device)
            inds = torch.cat([pos_inds, neg_inds], dim=0)

            new_proposals[img_id] = new_proposals[img_id][inds]
            new_labels[img_id] = new_labels[img_id][inds]

        gt_boxes = new_proposals
    ...
    def forward(
        ...
        if self.training:
            ...
            ######## THANHNT_UPDATE: START 
            gt_target, gt_label = self.select_target_samplse(proposals, targets, labels)
            gt_feature = self.box_roi_pool(features, gt_target, image_shapes)
            gt_feature = self.box_head(gt_feature)
            losses = {"loss_classifier": loss_classifier, "loss_box_reg": loss_box_reg, "gt_feature": gt_feature, "gt_label": gt_label}
            ######## THANHNT_UPDATE: END 
        else:
            boxes, scores, labels = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)

            ######## THANHNT_UPDATE: START
            proposal_feature = self.box_roi_pool(features, boxes, image_shapes)
            proposal_feature = self.box_head(proposal_feature)
            ######## THANHNT_UPDATE: END
